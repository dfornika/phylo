       ...
       ($ :div {:style {:display "flex"
          ...
          ($ :div {:style {:display "flex"
             ...
             ($ :span {:style {:font-size "10px" :font-weight 700 :letter-spacing "0.08em"
                               :text-transform "uppercase" :color "#8893a2"
          ...
          ($ :div {:style {:display "flex"
             ...
             ;; ── Toggles ──
             ($ :div {:style group-style}
                ($ :div {:style (merge section-style {:gap "10px" :flex-wrap "wrap"})}
             ...
             ($ :div {:style group-style}
                ($ :div {:style section-style}
                   ($ :button {:disabled (nil? active-reference-node-id)
                               ...
                               :style {:font-family toolbar-font :font-size "12px" :font-weight 600
                                       :color (if (nil? active-reference-node-id) "#999" navy)
                                       :padding "2px 8px"
                                       :cursor (if (nil? active-reference-node-id) "default" "pointer")
                                       :background "#f0f2f5"
                                       :border (str "1px solid " (if (nil? active-reference-node-id) "#ccc" navy))
                                       :border-radius "6px"}}
                      "Re-root Tree")
                   ($ :button {:disabled (not (or newick-str parsed-tree))
                               ...
                               :style {:font-family toolbar-font :font-size "12px" :font-weight 600
                                       :color navy :padding "2px 8px" :cursor "pointer"
                                       :background "#f0f2f5"
                                       :border (str "1px solid " navy)
                                       :border-radius "6px"}}
                      "Ladderize ↓")
